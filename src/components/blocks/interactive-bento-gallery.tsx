...
const MediaItem = ({ item, className, onClick }: { item: MediaItemType; className?: string; onClick?: () => void }) => {
  return (
    <div className={`${className} relative cursor-pointer group`} onClick={onClick}>
      <Image
        src={item.url}
        alt={item.title}
        fill
...
const GalleryModal = ({ selectedItem, isOpen, onClose, setSelectedItem, mediaItems }: GalleryModalProps) => {
  ...
  return (
    <>
      ...
      >
        <motion.div className="relative rounded-xl bg-sky-400/20 backdrop-blur-xl 
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <AnimatePresence mode="wait">
        {selectedItem ? (
          ...
          <motion.div
            className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4 auto-rows-[200px]"
            initial="hidden"
            animate="visible"
            exit="hidden"
            variants={{
          ...
          >
            {items.map((item, index) => (
              <motion.div
                ...
                className={`relative overflow-hidden rounded-xl cursor-move ${item.span} hover:shadow-xl transition-shadow duration-300`}
